...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($email && $pseudo && $nom && $type) {
        $stmt = $conn->prepare("UPDATE Utilisateurs SET Pseudo = :pseudo, Nom = :nom, Type = :type, Mot_de_passe = IF(LENGTH(:mot_de_passe) > 0, :mot_de_passe, Mot_de_passe) WHERE Adresse_email = :email");
